...
class FinalAnalyzer:
    ...
    def build_dataframe(self):
        ...
        for (pr, ts), data in self.results.items():
            ...
            if 'clean' in data and 'test' in data['clean']:
                ...
                row['clean_acc'] = clean.get('accuracy', np.nan)
                row['clean_f1'] = clean.get('f1_score', np.nan)
            ...
            if 'backdoored' in data:
                ...
                if 'test' in bd:
                    test = bd['test']
                    row['backdoor_acc'] = test.get('accuracy', np.nan)
                    row['backdoor_f1'] = test.get('f1_score', np.nan)
                if 'attack_metrics' in bd:
                    am = bd['attack_metrics']
                    row['asr'] = am.get('attack_success_rate', np.nan)
                    row['acc_backdoored_malware'] = am.get('acc_backdoored', np.nan)

            # Defenses
            for def_name in defenses:
                if def_name in data and 'test' in data[def_name]:
                    test = data[def_name]['test']
                    row[f'{def_name}_acc'] = test.get('accuracy', np.nan)
                    row[f'{def_name}_f1'] = test.get('f1_score', np.nan)

            # === METRICS CALCULATION (IDENTICAL TO ORIGINAL) ===
            if 'clean_acc' in row and 'backdoor_acc' in row:
                clean_acc = row['clean_acc']
                backdoor_acc = row['backdoor_acc']
                
                # Accuracy change: (backdoor - clean) / clean * 100
                # Positivo = backdoor MIGLIORE del clean (ultra stealth!)
                # Negativo = backdoor PEGGIORE (normale, detectable)
                row['acc_change_pct'] = ((backdoor_acc - clean_acc) / clean_acc) * 100
                
                # Mantieni anche acc_drop_pct per compatibilità (valore opposto)
                row['acc_drop_pct'] = -row['acc_change_pct']
                
                # Recovery calculation
                acc_diff = clean_acc - backdoor_acc
                
                if abs(acc_diff) > 1e-6:
                    for def_name in defenses:
                        acc_key = f'{def_name}_acc'
                        if acc_key in row and pd.notna(row[acc_key]):
                            defense_acc = row[acc_key]
                            recovered = defense_acc - backdoor_acc
                            row[f'{def_name}_recovery_pct'] = (recovered / acc_diff) * 100
                else:
                    for def_name in defenses:
                        row[f'{def_name}_recovery_pct'] = 100.0

            # F1 metrics
            if 'clean_f1' in row and 'backdoor_f1' in row:
                clean_f1 = row['clean_f1']
                backdoor_f1 = row['backdoor_f1']
                
                row['f1_drop'] = clean_f1 - backdoor_f1
                row['f1_drop_pct'] = (row['f1_drop'] / clean_f1) * 100
                
                if abs(row['f1_drop']) > 1e-6:
                    for def_name in defenses:
                        f1_key = f'{def_name}_f1'
                        if f1_key in row and pd.notna(row[f1_key]):
                            recovered_f1 = row[f1_key] - backdoor_f1
                            row[f'{def_name}_f1_recovery_pct'] = (recovered_f1 / row['f1_drop']) * 100
    ...
    def generate_plots(self, save_dir="Results/analysis_plots"):
        ...
        # 2. Stealthiness (Accuracy) - INTUITIVE VERSION
        ...
        for pr in sorted(self.df['poison_rate_pct'].unique()):
            ...
            # ✓ INTUITIVE: acc_change_pct - positivo = migliore = stealth!
            plt.plot(sub['trigger_size'], sub['acc_change_pct'], 
            ...
            for _, row in sub.iterrows():
                txt = f"{row['acc_change_pct']:+.2f}%"
                plt.annotate(txt,
                             xy=(row['trigger_size'], row['acc_change_pct']),
        ...
        plt.ylabel('Accuracy Change: Backdoor vs Clean (%)', fontsize=15, fontweight='bold', labelpad=10)
        plt.title('Backdoor Attack Stealthiness\n(Positive = Backdoor BETTER than Clean = Ultra Stealth! | Negative = Worse = Detectable)', 
        ...
        # 2-bis. F1-Score Stealthiness - INTUITIVE
        plt.figure(figsize=(12, 8))
        ...
        for pr in sorted(self.df['poison_rate_pct'].unique()):
            ...
            if 'f1_drop_pct' in sub.columns:
                # Inverti il segno per renderlo intuitivo
                f1_change_pct = -sub['f1_drop_pct']
                
                plt.plot(sub['trigger_size'], f1_change_pct, 
                         'o-', color=color, linewidth=4.5, markersize=14, 
                         markerfacecolor=color, markeredgecolor='white', markeredgewidth=2.5,
                         label=f'Poison Rate {pr:.0f}%', alpha=0.95)

                for idx, row in sub.iterrows():
                    change_val = -row['f1_drop_pct']
                    txt = f"{change_val:+.2f}%"
                    plt.annotate(txt,
                                 xy=(row['trigger_size'], change_val),
                                 xytext=(0, 15), textcoords='offset points',
                                 ha='center', va='bottom',
                                 fontsize=13, fontweight='bold', color=color,
                                 bbox=dict(facecolor='white', edgecolor=color, 
        ...
        plt.axhline(0, color='black', linewidth=3, alpha=0.9, zorder=1)
        plt.xlabel('Trigger Size', fontsize=15, fontweight='bold', labelpad=10)
        plt.ylabel('F1-Score Change: Backdoor vs Clean (%)', fontsize=15, fontweight='bold', labelpad=10)
        plt.title('Backdoor Attack Stealthiness - F1-Score\n(Positive = Backdoor BETTER = Ultra Stealth! | Negative = Worse = Detectable)', 
                  fontsize=18, fontweight='bold', pad=30)
        plt.legend(title='Poison Rate', fontsize=13, title_fontsize=14, 
                   loc='upper left', frameon=True, fancybox=True, shadow=True)
        plt.grid(True, alpha=0.4, linestyle='-', linewidth=1.2)
        ...
        sc = plt.scatter(self.df['acc_change_pct'],  # CHANGED: use change instead of drop
        ...
        for _, r in self.df.iterrows():
            plt.annotate(f"P{int(r.poison_rate_pct)} T{r.trigger_size}",
                        (r.acc_change_pct, r.acc_backdoored_malware*100),
                        fontsize=9, alpha=0.8)
        plt.axvline(0, color='black', linewidth=1.5)
        plt.axhline(50, color='red', linestyle='--', alpha=0.7, label='50% danger')
        plt.xlabel('Accuracy Change (%) ← Worse (Detectable) | Better (Stealth) →')
        plt.ylabel('Backdoored Malware → Benign (%) ↑ more dangerous')
        plt.title('Backdoor Trade-off: Stealth vs Real Danger\nIdeal attack: top-right (better accuracy + high danger)', fontweight='bold', pad=20)
        plt.legend()
